...
device = torch.device(
    'cuda') if DefaultConfig.InferWithGPU else torch.device('cpu')
MyModel.load_state_dict(torch.load(
    DefaultConfig.CkptPath, map_location=torch.device('cpu'))['state_dict'])
...
def inference(cvImg):
    ...
    msg = '该物体类别：' + index_to_class[predicted]
    msg1 = '该物体属于：' + index_to_group[predicted]
    print(msg)
    print(msg1)


if __name__ == '__main__':
    image = cv2.imread('static/uploads/temp.jpg')
    inference(image)
